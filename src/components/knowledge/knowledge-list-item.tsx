...
export function KnowledgeListItem({
  ...
  return (
    <Link
      className="block rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition hover:border-teal-200 hover:bg-teal-50/40 focus:outline-none focus:ring-2 focus:ring-teal-600/20 sm:p-5"
      href={`/app/items/${item.id}`}
    >
      <article aria-label={viewModel.title} className="min-w-0">
        <div className="flex min-w-0 items-start justify-between gap-4">
          <h2 className="min-w-0 text-base font-semibold leading-6 text-slate-950">
            {viewModel.title}
          </h2>
